...
function PaymentPanel({ supabase, session, setSession }) {
  ...
  async function ensureAccountAndContinue(e) {
    ...
    try {
      ...
      setMessage('Account ready. Choose QR Code or UPI ID to make payment.');
    } catch (err) {
  ...
  return <section id="payment" className="section paymentFirst"><div className="wrap"><h2 className="sectionTitle">Join Course & Pay Securely</h2><p className="sub">Fill your details first. After clicking Pay Now, QR or UPI ID appears instantly based on your selection. After payment, submit UTR and screenshot.</p>{status && paymentStep !== 'success' && <div className={`status ${status}`}>Latest payment status: {status.toUpperCase()}{status === 'approved' ? ' — Course unlocked.' : status === 'pending' ? ' — Admin approval pending.' : ' — Please contact support.'}</div>}<div className="payGrid securePayGrid"><div className="card"><div className="stepBadge">Step 1</div><h3>Student Details + Plan</h3><form className="form" onSubmit={ensureAccountAndContinue}><div><label className="label">Plan</label><select className="input" value={form.plan} onChange={e => updatePlan(e.target.value)}><option>Starter ₹199</option><option>Growth ₹299</option><option>Pro ₹499</option><option>Lifetime ₹1999</option></select></div><div><label className="label">Name</label><input className="input" required value={form.name} onChange={e => update('name', e.target.value)} placeholder="Student full name" /></div><div><label className="label">Email ID</label><input className="input" required type="email" value={form.email} onChange={e => update('email', e.target.value)} placeholder="student@email.com" /></div><div><label className="label">Password</label><input className="input" required type="password" minLength="6" value={form.password} onChange={e => update('password', e.target.value)} placeholder="Create or enter password" /></div><div><label className="label">Mobile Number</label><input className="input" required value={form.phone} onChange={e => update('phone', e.target.value)} placeholder="10 digit mobile number" /></div><div className="toggleRow"><button type="button" className={`choiceBtn ${authMode === 'auto' ? 'selected' : ''}`} onClick={() => setAuthMode('auto')}>New Student / Auto</button><button type="button" className={`choiceBtn ${authMode === 'login' ? 'selected' : ''}`} onClick={() => setAuthMode('login')}>Already Registered</button></div><button className="btn payNowBtn" type="submit">Pay Now ₹{form.amount}</button>{session && <div className="notice successNotice">Logged in as {session.user.email}</div>}</form></div><div className={`card finalPayCard ${paymentStep === 'method' || paymentStep === 'pay' ? 'slideInPay' : ''}`}>{paymentStep === 'details' && <div className="cleanPaymentIntro"><div className="stepBadge tealBadge">Step 2</div><h3>Final Payment</h3><p>QR and UPI details will appear here after you fill details and click <b>Pay Now</b>.</p></div>}{paymentStep === 'method' && <div className="animatedPayment"><div className="pulseIcon">💳</div><div className="stepBadge tealBadge">Step 2</div><h3>Choose Payment Mode</h3><p className="sub">Select one option. The final payment section opens instantly.</p><div className="methodGrid"><button className="methodCard" type="button" onClick={() => selectMethod('qr')}><span>📷</span><b>Pay via QR Code</b><small>Scan with any UPI app</small></button><button className="methodCard" type="button" onClick={() => selectMethod('upi')}><span>🏦</span><b>Pay via UPI ID</b><small>Copy UPI ID and pay</small></button></div></div>}{paymentStep === 'pay' && <div className="animatedPayment"><div className="stepBadge tealBadge">Step 3</div><h3>{method === 'qr' ? 'Scan QR Code' : 'Pay Using UPI ID'}</h3><div className="qrBox secureQr revealPayBox">{method === 'qr' && <img className="qr" src={QR_IMAGE} alt="Bapi Debnath UPI QR Code" />}{method === 'upi' && <div className="upiOnly"><div className="upiBig">{UPI_ID}</div><button className="btn secondary" type="button" onClick={() => navigator.clipboard.writeText(UPI_ID)}>Copy UPI ID</button></div>}<p className="sub">Pay ₹{form.amount} for {form.plan}. Use Google Pay, PhonePe, Paytm, BHIM or any UPI app.</p></div><form className="form" onSubmit={submit}><div><label className="label">UPI Transaction ID / UTR</label><input className="input" required value={form.transaction_id} onChange={e => update('transaction_id', e.target.value)} placeholder="Example: 412345678901" /></div><div><label className="label">Payment Screenshot</label><input className="input" required type="file" accept="image/*,.pdf" onChange={e => setFile(e.target.files?.[0] || null)} /></div><button className="btn teal" type="submit">Submit Payment Proof</button><button className="btn secondary" type="button" onClick={() => setPaymentStep('method')}>Change Payment Option</button></form></div>}{paymentStep === 'success' && <div className="paymentSuccess"><div className="successCircle">✓</div><h2>Payment Proof Submitted</h2><p>Your account is created and your payment proof is now pending admin approval.</p><div className="successDetails"><div><b>Plan:</b> {form.plan}</div><div><b>Amount:</b> ₹{form.amount}</div><div><b>Status:</b> Pending Approval</div></div><p className="sub">Redirecting to dashboard in {redirectSeconds} seconds...</p><button className="btn" type="button" onClick={() => document.getElementById('dashboard')?.scrollIntoView({ behavior: 'smooth' })}>Go to Dashboard Now</button></div>}{message && <div className="notice">{message}</div>}</div></div></div></section>;
}
